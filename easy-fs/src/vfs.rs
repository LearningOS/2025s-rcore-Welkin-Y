...
impl Inode {
    ...
    /// Get file status, return (ino, mode, nlink)
    pub fn fstat(&self) -> Result<(u64, u32, u32), &'static str> {
        info!("fstat inode {}", self.inode_id);
        let ino = self.inode_id as u64;
        let mode = self.read_disk_inode(|disk_inode| {
    ...
    pub fn linkat(&self, _new_path: &str) -> Result<(), &'static str> {
        ...
        root_node.modify_disk_inode(|root_inode| {
            ...
            // write dirent
            let dirent = DirEntry::new(_new_path, self.inode_id as u32);
            root_inode.write_at(
                file_count * DIRENT_SZ,
                dirent.as_bytes(),
                &self.block_device,
        ...
        self.modify_disk_inode(|disk_inode| {
            disk_inode.nlink += 1;
            info!("Incremented inode nlink: {}", disk_inode.nlink);
        });
        block_cache_sync_all();
        Ok(())
    }
    ...
    /// Unlink at
    pub fn unlinkat(&self, _path: &str) -> Result<(), &'static str> {
        info!("unlinkat {}", _path);
        ...
        if self.read_disk_inode(|disk_inode| disk_inode.nlink == 0) {
            drop(_fs);
            self.clear();
        }
        block_cache_sync_all();
